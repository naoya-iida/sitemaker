...
if st.button("生成する"):
    ...
    response = openai.ChatCompletion.create(
        ...
        messages=[
            {"role": "system", "content": "あなたはプロのトラベルライターです。施設紹介文を作成してください。以下のフォーマットに従って、各セクションを分けて出力してください。『{facility_name}へようこそ！』という文章から始めてください。OTAキャッチコピーがある場合は考慮に入れて、キャッチコピーはキーワード『[キャッチコピーキーワード1]』、『[キャッチコピーキーワード2]』、『[キャッチコピーキーワード3]』、『[キャッチコピーキーワード4]』を基に50文字以内の魅力的な文章に仕上げてください。この宿の魅力をキーワード『[キーワード1]』、『[キーワード2]』、『[キーワード3]』を基に、それぞれの特徴を詳しく説明するタイトルと文章を3つ作成してください。文章には名称を含めず、タイトルは太字に。館内での過ごし方のキーワード『[キーワード1]』、『[キーワード2]』、『[キーワード3]』を基に、それぞれの設備やサービスを詳しく説明するタイトルと文章を3つ作成してください。補足情報がある場合は、それを考慮に入れてください。文章には名称を含めず、タイトルは太字に。周辺エリアの見どころは各見どころの名称をそのままタイトルとして使用し、自由記述を基にそれぞれの魅力を詳しく説明する文章を作成してください。文章には名称を含めず、タイトルは太字に。周辺の人気グルメ。各グルメの名称をそのままタイトルとして使用し、自由記述を基にそれぞれの料理や雰囲気を詳しく説明する文章を作成してください。文章には名称を含めず、タイトルは太字に。感嘆符は使用せず、トーンとマナーを統一し、簡潔でわかりやすく、かつ読み応えのある内容にしてください。ただし、ウェルカムメッセージには感嘆符を使用します。"},
            {"role": "user", "content": f"""
            施設名: {facility_name}
            OTAキャッチコピー: {ota_copy}
            貸切風呂の魅力: {keyword1}
